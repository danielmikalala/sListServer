...
exports.handlerAddMember = (req, res) => {
  try {
    ...
    const userExists = users.some((u) => u.id === userIdStr);
    if (!userExists) {
...
exports.handlerRemoveMember = (req, res) => {
  try {
    ...
    const idx = list.members.indexOf(userId);
    if (idx === -1) {
...
exports.handlerAddItem = (req, res) => {
  try {
    ...
    if (list.items.includes(itemId))
      return res.status(400).json({ error: "Item already added" });
...
exports.handlerRemoveItem = (req, res) => {
  try {
    ...
    if (!list) return res.status(404).json({ error: "List not found" });
    list.items = list.items || [];
    const idx = list.items.indexOf(itemId);
